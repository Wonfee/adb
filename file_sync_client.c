...
int do_sync_push(const char *lpath, const char *rpath, int verifyApk)
{
    ...
    if(S_ISDIR(st.st_mode)) {
	    ...
	    int name_len=GBKToUTF8(name,NULL,0);
	    name_len=GBKToUTF8(name,utf8name,name_len);
	    tmplen=strlen(utf8name)+strlen(rpath)+2;
    	#endif	
	...
	#ifdef USE_MINGW
	    snprintf(tmp, tmplen, "%s/%s", rpath, utf8name);
	#endif
...
#ifdef USE_MINGW
	char gbk_name[260];
	int name_len=UTF8ToGBK(NULL,name,0);
	name_len=UTF8ToGBK(gbk_name,name,name_len);
#endif	
...
#ifdef USE_MINGW
	ci = mkcopyinfo(args->rpath, args->lpath, name, gbk_name,1);
#endif
...
#ifdef USE_MINGW
	ci = mkcopyinfo(args->rpath, args->lpath, name, gbk_name,0);
#endif
        ci->time = time;
        ci->mode = mode;
        ci->size = size;
...
int do_sync_pull(const char *rpath, const char *lpath)
{
    ...
    if(S_ISREG(mode) || S_ISLNK(mode) || S_ISCHR(mode) || S_ISBLK(mode)) {
	    ...
	    int name_len=UTF8ToGBK(NULL,name,0);
	    name_len=UTF8ToGBK(gbk_name,name,name_len);
	    tmplen=strlen(gbk_name)+strlen(lpath)+2;
    	#endif	
                char *tmp = malloc(tmplen);
                if(tmp == 0) return 1;
	...
	#ifdef USE_MINGW
 		snprintf(tmp, tmplen, "%s/%s", lpath, gbk_name);
	#endif
